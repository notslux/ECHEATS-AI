...
namespace AimmyWPF.UserController
{
    ...
    public partial class AInfoSection : UserControl
    {
        static async Task<(string, string)> GetLatestReleaseInfo()
        {
            ...
            {
                client.DefaultRequestHeaders.Add("User-Agent", "ECHEATS-AI"); // GitHub API requires a user-agent

                var response = await client.GetStringAsync("https://api.github.com/repos/notslux/ECHEATS-AI/releases");
        ...
        static async Task DoUpdate(string currentVersion)
        {
            ...
            string localZipPath = Path.Combine(envTempPath, "ECHEATSUpdate.zip");
            ...
            string extractPath = Path.Combine(envTempPath, "ECHEATSUpdate");
